...
void hysteria2Construct(Proxy &node, const std::string &group, const std::string &remarks, const std::string &add, const std::string &port, const std::string &type, const std::string &auth, const std::string &host, const std::string &up, const std::string &down, const std::string &alpn, const std::string &obfsParam, const std::string &insecure ,tribool udp, tribool tfo, tribool scv, tribool tls13)
{
...
void explodeClash(Node yamlnode, std::vector<Proxy> &nodes)
{
    ...
    for(uint32_t i = 0; i < yamlnode[section].size(); i++)
    {
        ...
        case "hysteria2"_hash:
            ...
            hysteria2Construct(node, group, ps, server, port, type, auth, host, up, down, alpn, obfsParam, insecure, udp, tfo, scv);
            break;
        default:
            continue;
        }
...
void explodeStdHysteria2(std::string hysteria2, Proxy &node)
{
    ...
    hysteria2Construct(node, HYSTERIA2_DEFAULT_GROUP, remarks, add, port, type, auth, host, up, down, alpn, obfsParam, insecure);
    return;
}
